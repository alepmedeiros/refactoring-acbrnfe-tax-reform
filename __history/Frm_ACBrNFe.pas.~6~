...
type
  ...
  private
    ...
    {com reforma direto}
    procedure EmitirNFCe_SP_Restaurante(NumDFe: String);
  public
    { Public declarations }
  end;
...
begin
  ...
  EmitirNFCe_SP_Restaurante(vAux);
...
procedure TfrmACBrNFe.EmitirNFCe_SP_Restaurante(NumDFe: String);
var
  Ok: Boolean;
  BaseCalculo,
  ValorICMS: Double;
begin
  ...
  begin
    ...
    Ide.tpNF      := tnSaida;
    Ide.tpEmis    := teNormal;
    Ide.tpAmb     := taHomologacao; // trocar para taProducao em produção
    Ide.cUF       := 35;            // SP
    Ide.cMunFG    := 3550308;       // São Paulo
    Ide.finNFe    := fnNormal;
    Ide.tpImp     := tiNFCe;
    Ide.indFinal  := cfConsumidorFinal;
    Ide.indPres   := pcPresencial;    // venda balcão
    Ide.indIntermed := iiSemOperacao; // não é marketplace

    // Reforma Tributária (campos RTC)
    Ide.cMunFGIBS := 3550308;
    Ide.tpNFDebito := tdNenhum;
    Ide.tpNFCredito := tcNenhum;

    //================ EMITENTE =======================
    Emit.CNPJCPF := '12345678000195';  // fictício
    Emit.IE      := '111111111111';    // fictício
    Emit.xNome   := 'RESTAURANTE TESTE LTDA';
    Emit.xFant   := 'RESTAURANTE TESTE';
    Emit.EnderEmit.xLgr := 'RUA EXEMPLO';
    Emit.EnderEmit.nro  := '100';
    Emit.EnderEmit.xBairro := 'CENTRO';
    Emit.EnderEmit.cMun := 3550308;
    Emit.EnderEmit.xMun := 'SAO PAULO';
    Emit.EnderEmit.UF   := 'SP';
    Emit.EnderEmit.CEP  := 12345000;
    Emit.EnderEmit.cPais := 1058;
    Emit.EnderEmit.xPais := 'BRASIL';
    ...
    begin
      ...
      Prod.cProd  := '001';
      Prod.cEAN   := 'SEM GTIN';
      Prod.xProd  := 'REFEICAO SELF-SERVICE';
      Prod.NCM    := '21069090';
      Prod.CFOP   := '5102';
      Prod.uCom   := 'UN';
      Prod.qCom   := 1;
      ...
      begin
        ...
        with PIS do
        begin
          CST := pis49; vBC := 0; vPIS := 0;
        end;

        // COFINS
        with COFINS do
        begin
          CST := cof49; vBC := 0; vCOFINS := 0;
        end;

        // Reforma Tributária — valores fictícios para validar layout
        with IBSCBS do
        begin
          CST := cst000;
          cClassTrib := '000001';
          gIBSCBS.vBC  := 25.00;
          gIBSCBS.vIBS := 1.25;
          gIBSCBS.gCBS.pCBS := 2.00;
          gIBSCBS.gCBS.vCBS := 0.50;
        end;
    ...
    //================ TOTAIS =========================
    Total.ICMSTot.vProd := 25.00;
    Total.ICMSTot.vNF   := 25.00;

    Total.IBSCBSTot.vBCIBSCBS := 25.00;
    Total.IBSCBSTot.gIBS.vIBS := 1.25;
    Total.IBSCBSTot.gCBS.vCBS := 0.50;
    ...
    //================ INFO ADICIONAIS ================
    InfAdic.infCpl := 'DOCUMENTO EMITIDO EM HOMOLOGACAO - SEM VALOR FISCAL';
  end;
